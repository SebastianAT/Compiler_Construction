...
public class Decoder {
	...
	/** Interpret integer as 4 ASCII characters (bytes).
	 * MSB is first character.
	 * If byte is not printable, print a . (dot).
	 */
	private static String int2chars(int n) {
		for (int i=0; i < 4; i++) {
			int c = (n>>(24-i*8)) & 0xff;
			sb.setCharAt(i, (char) (c < 32 || c > 126 ? '.' : c));
		}
		return sb.toString();
	}
	...
	public static void decode(byte[] c, int off, int len) {
		...
		while (cur < len) {
			switch(get()) {
				...
				case sprint:	P("sprint "+get2()); break;
	...
	/**
	 * Decode len integers of static data area starting at offset.
	 */
	public static void decodeStaticData(int[] data, int offset, int len)
	{
		cur = offset;
		for (int i=0; i < len; i++, cur++) {
			int n = data[cur];
			System.out.format("%1$5d: %2$10d (0x%2$08x) | %3$s\n", cur, n, int2chars(n));
		}
	}
